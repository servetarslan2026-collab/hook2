<script lang="ts">
  ...
  let isAdmin = $derived(($auth.user as any)?.is_admin === true);
      ...
      <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
        ...
        {#if isAdmin}
          <a href="/admin" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm transition-colors"
            style="{$page.url.pathname.startsWith('/admin') ? 'background: var(--bg-hover); color: white;' : 'color: var(--danger);'}">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="pt-4 mt-4 border-t" style="border-color: var(--border);">
          ...
          <a href="/app/event-types" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm"
            style="color: var(--text-secondary);">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <a href="/app/settings" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm"
            style="color: var(--text-secondary);">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.066 2.573c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.573 1.066c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.066-2.573c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/>
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
          ...
          <a href="/app/signature-test" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm"
            style="color: var(--text-secondary);">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/>
            </svg>
